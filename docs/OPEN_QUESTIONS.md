...
| Postcheck / validation | `dba_registry_sqlpatch` STATUS check in `postcheck.yml` is informational-only (`debug`, not `failed_when`) — a real per-CDB patch failure there wouldn't currently fail the play, so the Executor would see `apply_result.ok = True` even if a CDB's patch actually failed at the SQL level | ❌ **Known gap**, already tracked in `STATUS.md` Open Issues (base-repo issue, not agentic-specific — but the Executor inherits it) |
